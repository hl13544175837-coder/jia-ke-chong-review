...
Agent 简历导入采用最小双库边界：外部 Agent 单向把在线简历、完整聊天快照或完整简历推入智聘；智聘不启动或控制 Agent。“在线简历”是招聘专员仅看本人数据的独立菜单，完整简历继续进入现有简历库；两个库不自动关联、搬移、覆盖或删除。月度人员视图只增加成功导入在线简历的客观份数，不做排名或自动绩效判断。
