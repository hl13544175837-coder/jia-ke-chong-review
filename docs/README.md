...
> **当前状态（2026-08-06）**：当前唯一前端主线为 `readdy-frontend/`，后端主线为 `backend/`，公司唯一发布代码源为 CFPD GitLab 的 `test` 分支。候选人详情、候选人库、需求候选人工作区、人才地图和简历后端已按模块边界治理；非核心 Mock 仍保留，Test/SIT 简历 AI 关闭。准确版本以当前工作树 `git log -1`、`git ls-remote cfpd refs/heads/test`、登录页版本号和后端 `/actuator/info` 为准；统一本地门禁入口为 `./scripts/check-sit-release.sh`。代码推送、Libra 构建和 SIT 部署必须分别现场核对，不在本文写死临时分支或提交号。
>
...
| 本地代码候选 | 当前实施工作树的 HEAD、diff 和统一门禁 | 当前本地 Test/SIT 发布候选；不自动证明已 push、已 Libra 或已 SIT |
...
| 15 | [12 5190 真实上线改造矩阵](./12_5190真实上线改造矩阵.md) | 当前 5190 各页面的演示数据、可复用接口、真实缺口和实施顺序 | 把 5190 从演示产品改成真实业务系统时必读 |
| 16 | [13 试点业务流程与研发接口交接](./13_试点业务流程与研发接口交接.md) | 用户确认的业务需求、HR/业务筛选、一面二面、评价字段、隐藏模块和研发接口责任 | 交给研发接接口前必读 |
...
Agent 简历导入采用最小双库边界：外部 Agent 单向把在线简历、完整聊天快照或完整简历推入智聘；智聘不启动或控制 Agent。“在线简历”在同一组织内向拥有 `candidates` 菜单权限的招聘专员、招聘主管和管理员共享；仅导入人可编辑，其他可见账号可删除，删除会记录审计事件。完整简历继续进入现有简历库；两个库不自动关联、搬移、覆盖或删除。月度人员视图只增加成功导入在线简历的客观份数，不做排名或自动绩效判断。
...
- 生产应用工厂不执行 DDL；空库显式 bootstrap，已有库 Alembic。当前 additive head 是 `20260811_18`：11 增加简历 SHA-256 指纹，12 增加简历版本留档，13 增加面试改约申请，14 增加本地 OA 结果登记字段和索引，15 增加原简历数据库副本，16 增加独立 `online_resumes` 在线简历表，17 增加人才地图人员字段，18 增加人才联系记录。Strict cutover、目标引擎恢复证据和 SIT 多角色现场验收仍属于环境发布门禁，不能由本地代码测试替代。
