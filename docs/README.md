...
- 生产应用工厂不执行 DDL；空库显式 bootstrap，已有库 Alembic。当前 additive head 是 `20260807_16`：11 增加简历 SHA-256 指纹，12 增加简历版本留档，13 增加面试改约申请，14 增加本地 OA 结果登记字段和索引，15 增加原简历数据库副本，16 增加独立 `online_resumes` 在线简历表。Strict cutover、目标引擎恢复证据和 SIT 多角色现场验收仍属于环境发布门禁，不能由本地代码测试替代。
