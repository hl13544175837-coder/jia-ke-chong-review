...
| `base_agent/image_resume_parser.py` | 视觉简历解析器支持钥匙串 API Key，避免上传 PDF / DOCX / 图片简历时把 `keychain:` 字符串当作真实 key |
...
| `candidate_merges` | `CandidateMerge` | `org_id`, `primary_candidate_id`, `duplicate_candidate_id`, `merged_by`, `reason`, `created_at` | 重复档案软合并审计；重复档只允许记录一次，保留主档、操作人和原因 |
...
| `GET/PATCH/DELETE` | `/online-resumes[/<id>]` | recruiter/manager/admin，仅当前组织 | 列表支持 `demand_id`、`gender`、`age_from`、`age_to`、`created_from`、`created_to`、`source_platform`、`education_level`、`location`、`keyword`、`owner_hr_id` 筛选；三类角色在同组织内共享列表和详情，其他组织和面试官失败。时间使用 ISO 格式并先归一为 UTC；PATCH 仅允许 `owner_hr_id` 对应的导入人，DELETE 允许上述三类同组织角色并写 `online_resume.deleted` 审计事件；完整聊天只由 Agent 整体更新 |
| `GET` | `/online-resumes/owner-options` | recruiter/manager/admin，仅当前组织 | 返回当前组织在线简历实际导入人，用于共享列表的导入人筛选 |
| `GET` | `/online-resumes/demand-options` | recruiter/manager/admin，仅当前组织 | 返回当前组织在线简历库实际出现的招聘需求，确保共享列表能筛选其他导入人的需求 |
| `POST` | `/resume/batches/<batch_id>/rollback` | 批次上传人/manager/admin | 撤回误导入批次，候选人软删除、匿名化、删除原文件并写审计 |

在线简历最小导入请求必须是带 `items` 的 JSON 对象，不能直接发送数组；`demand_id` 必须是提示词中当前招聘需求的固定 ID：
...
```json
{
  "items": [
    {
      "demand_id": 20,
      "external_record_id": "boss:候选人唯一ID",
      "boss_account": "何龙",
      "name": "候选人姓名",
      "source_platform": "BOSS直聘",
      "resume_text": "候选人在线资料完整原文",
      "chat_json": [
        {
          "sender": "候选人",
          "text": "消息内容",
          "sent_at": "2026-08-08T12:00:00+08:00"
        }
      ]
    }
  ]
...
极简模式可省略 `resume_json`，服务端会根据顶层字段和 `resume_text` 生成 `extracted_info` 并把完整原文写入 `raw_text`；如果显式传入，也必须同时保留 `raw_text`。
...
| `POST` | `/interview/feedback` | 登录且有 Demand/assignment 权限 | 当前评价必须在 assignment 确认为 `awaiting_feedback` 后提交；HR 或所属面试官可在面试开始后确认，未确认返回 `409 interview_not_confirmed`，未到开始时间返回 `409 interview_not_started`；同 assignment 重复返回已有反馈，数据库唯一索引为并发最终防线；任何反馈都不推进流程 |
...
| `POST` | `/interview/assignments/<assignment_id>/mark-conducted` | recruiter/manager/admin + Demand 管理权，或该 assignment 所属面试官 | 面试开始后确认真实发生，将状态从 `scheduled` 原子改为 `awaiting_feedback` 并通知具体面试官；未到时间拒绝，重复调用幂等，其他面试官返回 403 |
...
- `/api/agent-imports/online-resumes` 把 Agent 挑选的在线简历、原始简历文本与完整聊天快照写入独立 `online_resumes` 表。每条导入必须明确携带当前账号有权管理的 active `demand_id`；缺失、无效或越权时逐条拒绝，禁止自动回退到第一个需求。列表在当前组织内向 recruiter/manager/admin 共享，可组合筛选；无 JSON 条件时由数据库直接分页，有年龄、性别、学历、城市或关键词条件时在统一服务层做兼容 MySQL/SQLite 的结构化过滤。仅导入人可人工编辑，同组织其他可见账号可删除，删除保留审计事件。
...
8. 5190 简历库可先上传到公司人才库而不绑定 Demand；后续选择具体 `demand_id`，调用 `/candidates/match/preview` 生成当前选择的岗位适配预览，再通过 `/candidates/pipeline/add` 单人或批量加入。预览返回命中标签、缺失标签、匹配分和目标 Demand 内的 `latest_stage`，不写入 `matches`；重新启用判断必须使用该 Demand 内阶段，不能使用候选人的全局最新阶段。搜索、学历、意向城市、技能关键词、最低技能分、来源、解析状态、活动/精确流程状态、入库日期范围、个人收藏和任一 Demand 当前阶段均在后端分页前筛选，不能只过滤当前页。已有筛选页的展示层统一为普通筛选在上、绿色状态分类在下、列表或表格最后；普通筛选桌面端使用 `160px × 36px` 控件和 `8px` 间距，可整组收起，状态分类不进入收缩容器。候选人库主列表本期不暴露技能列和技能筛选，但接口兼容能力保留。已淘汰候选人没有活动 Flow，因此属于公司人才库；最新状态为已入职或仅有已转出终态的档案不进入该可复用范围。重新加入原 Demand 时必须填写原因并追加新的 `pending` 流水。
...
- 前端：`readdy-frontend/src/pages/interviewer/interviews/page.tsx`
- 后端：`backend/app/api/interview.py`

面试官反馈写入 `interview_feedback`，候选人详情 journey 按 Demand 聚合流程时间线、AI 记录和面试反馈。结构化评价复用现有列：`strengths`、`concerns` 存文字，`evaluation_json` 存 `satisfaction`、`job_match`、`recommendation`，不新增表字段。满意时优势必填、不满意时顾虑必填、待定时补充备注必填，所有长文本上限 1000 字；旧的简单评价仍可读取和修改。面试官只提交反馈；候选人是否进入 Offer 或淘汰，由 HR/经理/管理员人工处理。

当前使用 `round_sequence` 表达 `interview` 内部轮次，不恢复“一面/二面/终面”主阶段。一面、二面、终面固定对应 1、2、3，前端不提供可编辑序号，后端对冲突组合返回 400 `round_sequence_mismatch`；其他面试类型使用计算出的下一轮。每轮可有多名参与者，但只允许一名有效主面试官；服务锁与稳定 409 提供业务响应，revision `20260711_02` 的 `primary_slot` 唯一索引提供并发最终防线。同一 assignment 的 feedback 也有唯一索引；重复提交返回已有反馈，不产生第二条。只有主面试官反馈能将该轮标记完成，仍不会推进主流程。创建与取消由 `interview_workflow_service` 统一管理：客户端不能写任意状态，未反馈任务填写原因后可取消并把 `primary_slot` 置空，已有反馈任务拒绝取消；通知、审计和 assignment 事实同事务提交。

同一 Demand 内，二面及后续轮次的面试官凭本人有效 assignment 读取 candidate journey。接口只返回本人当前轮及更早轮次，不返回后续轮次的安排、评价或汇总结论；本人当前轮尚未提交评价时，还会同时隐藏更早轮次的 `interview_rounds.feedback`、顶层 `feedback` 以及由评价生成的汇总结论，提交本人评价后才只读解锁。反馈更新接口仍只允许原反馈人本人修改，其他面试官返回 403。

候选人和面试详情展示层统一为“面试信息 / 候选人简历 / 面试评价”三个本地页签；切换页签不重复请求。抽屉底部固定任务状态和当前评价动作，列表查询条件直接展示，状态和按钮使用共享语义组件。评价弹窗存在时详情层不响应 Escape，加载或历程失败均保留错误和局部重试。面试官角色的 journey 响应仍由后端清空需求审批人员/原因、业务筛选、流程时间线、AI 预筛、淘汰与 Offer，并且只返回本人当前轮及此前轮次评价；前端 `interviewOnly` 模式只渲染面试过程，因此同一页面结构不会扩大角色权限。Offer 一期固定为“待登记 / 跟进中 / 已完成”，只登记 OA 编号、状态和备注等线下事实，OA 自动发起/同步尚未接入。

候选人列表的数据卫生提示由后端同组织、未删除记录计算：`identical_resume_count` 只统计非空 `resume_sha256`，`same_name_count` 只统计 NFKC 规范化后的非空姓名；两者均为只读提示，不触发合并、覆盖或删除。`is_local_demo_record` 只有 `LOCAL_SCHEMA_COMPAT=true` 且名称命中明确本地规则时为真，非本地环境固定为假。
...
| Agent 在线简历导入 | `pages/online-resumes/*`, `features/onlineResumes/*` | `api/agent_imports.py`, `api/online_resumes.py`, `services/online_resume_service.py` | 写独立 `online_resumes` 和完整聊天快照；同组织 recruiter/manager/admin 共享读，仅导入人可编辑，可见账号可删除；和正式候选人库不自动关联 |
...
| `online_resume_service` | 外部 Agent 在线简历批量导入、完整聊天快照、组织共享读、导入人编辑、组织内删除及审计/客观导入事件 |
