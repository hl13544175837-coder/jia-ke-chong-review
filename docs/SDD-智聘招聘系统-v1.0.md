...
8. 简历库筛选区使用“目标招聘需求”选择具体 `demand_id`，再使用该需求关联的 `job_id` 调用 `/jobs/<id>/match-preview` 生成岗位适配预览；预览只返回当前页候选人的命中标签、缺失标签和匹配分，不写入 `matches`。用户点击“加入所选需求”时必须同时传入 `demand_id`；“入流程状态”只区分候选人是否存在流程记录。列表展示的负责人姓名、最近阶段、需求编号、流程岗位和更新时间来自 `/candidates` 的只读摘要，不允许前端按候选人序号生成状态。
