...
> 状态：Codex 候选已完成第三轮“Demand 可点击筛选收口”。同页交互基线为 `4656696`，本轮真实筛选功能提交为 `c2dcf78`；准确文档 HEAD 以 `git log -1` 和 `CODEX_RESULT_2026-07-22.md` 为准。本文是迁移过程的接线真源，不代替已部署环境证明。
>
> 代码基线：`origin/test` 的 `5e251a2`；当前分支 `codex/readdy-test-product`；本轮功能提交 `c2dcf78`。旧公司招聘系统是视觉基准，Figma/Readdy 是流程与交互基准。
>
...
| `/demands` / `/jobs` 招聘管理 | recruiter/manager/admin | `/demands`、`/jobs`、`/jobs/clarify`、匹配接口 | Demand 表头和职位、编号、部门、城市、负责人、HC、截止日期、阶段数字均可触发真实服务端筛选；“查看详情”在当前页打开右侧抽屉。岗位/JD 模板位于 `/job-templates`，新增岗位和 AI 澄清/保存仍走真实接口 |
...
- Demand：五个信息表头均可展开内联筛选；职位和需求编号使用精确条件，部门、城市、负责人、HC、截止日期、阶段数字和状态均可一键触发真实服务端筛选，分页总数和总页数按筛选后的数据库结果计算。高级条件以紧凑标签显示并可单独清除，零结果仍可“清除全部筛选”恢复。独立“查看详情”按钮在右侧打开概览抽屉；新增 Demand、调整优先级、转派负责人、暂停/关闭和恢复继续复用真实 API、角色校验与审计，“匹配候选人”保留为抽屉次级入口。
...
- Demand：五个表头展开器均实点可用；职位 `4→1`、需求编号 `4→1`、部门 `4→1`、城市 `4→3`、负责人 `4→2`、HC 未达成 `4→3`、面试阶段 `4→2`。截止日期条件由真实接口执行并显示活动标签；输入不存在的精确职位会得到真实空态，清除后恢复 4 条。独立“查看详情”在 `/demands` 原地址打开“数据分析师”右侧抽屉。
...
- 本轮仅扩展 Demand 列表的精确职位/编号、截止日期、HC 和当前阶段查询，不新增迁移；筛选在数据库分页前执行，并复用现有组织隔离、候选人最新阶段和 legacy 单 Demand 口径。冻结鉴权、权限、Apollo 和 Token 文件未改。
