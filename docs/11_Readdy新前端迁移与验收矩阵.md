...
- 面试官招聘需求、待面试官筛选和我的面试把状态页签、搜索条件及必要筛选写入当前地址；从左侧菜单离开再返回时继续保留，长列表继续复用主壳滚动位置记忆。
...
- 面试官任务详情固定为“面试信息 / 候选人简历 / 历史评价”三页签；Tabbit 实测三个页签均可切换，底部任务状态和“修改评价”等当前动作始终保留，切换页签不触发写操作。
