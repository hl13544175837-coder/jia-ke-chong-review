...
| `/candidates` 简历库 | recruiter/manager/admin | `/candidates`、`/resume/*`、收藏、查重合并、匹配预览和批量入流程 | 支持先入公司人才库再匹配 Demand；全部、活动流程、公司人才库、个人收藏四个范围和多维条件均在分页前真实筛选。普通筛选以 `160px × 36px` 紧凑控件置于上方并可整组收起，范围页签固定在其下且始终显示；流程状态、学历、意向城市和当前阶段保留表头筛选，日期范围、来源、解析状态和排序置顶且不重复。当前阶段与兼容简历学历真实返回；单人/批量收藏、加入流程、淘汰后带原因重新启用已接通；manager/admin 可按完整联系方式安全合并无业务历史的重复档案 |
...
| `/interviews` 面试管理 | recruiter/manager/admin | `/interviews`、`/interview/assignments`、取消、反馈、AI 面试 | 统计卡在当前页筛选，面试行打开右侧详情（简历/招聘流程）；安排、取消、反馈仍是受权限控制的真实操作；支持 `status` 和 `demand` URL 筛选 |
...
- 当前 Alembic head 为 `20260729_12`；08 补齐试点审批与业务筛选 schema，09 新增个人收藏与候选人合并审计表，10 新增持久化组织设置和用户部门字段，11 新增可空简历 SHA-256 指纹和组织级普通索引，12 新增候选人简历历史版本表。
...
- 候选人：点击行或姓名打开同页详情；概览和当前/历史应聘流程来自真实候选人流程接口，完整档案只作为次级入口。搜索、学历、城市、来源、解析状态、精确流程状态、阶段和入库起止日期在数据库分页前生效；流程状态、学历、意向城市和活动阶段由表头展开，低频条件置顶，信息值可点击复用同一筛选。主列表本期隐藏核心技能、技能关键词和最低技能分入口，后端兼容和岗位匹配能力保留到二期交互。
...
### 12.6 2026-07-30 本地交互连续性第一批

- 面试官招聘需求、候选人筛选和我的面试把状态页签、搜索条件及必要筛选写入当前地址；从左侧菜单离开再返回时继续保留，长列表继续复用主壳滚动位置记忆。
- 页面记忆只保留页签、搜索和筛选，不保留 `demand` 详情、`task`、`candidate`、`assignment`、`offer` 等临时详情或操作参数，避免返回页面时自动弹出旧侧栏。
- 面试官招聘需求整行可打开真实只读详情；招聘需求、业务筛选、面试简历和 Kanban 流程历史等只读侧栏在桌面端只遮内容区，左侧菜单和顶部账号区仍可操作。评价、审核、推进等会写数据的弹窗继续保持模态锁定。
- Kanban 最后选择的 Demand 只在当前浏览器会话保存需求编号，用于从 `/kanban` 基础地址重新进入时恢复视图；不在浏览器保存候选人、流程或其他业务数据，后端仍是业务事实唯一来源。
- 本地验收使用 `http://127.0.0.1:5190`、后端 `:5010` 和登录桥 `:5100`，未新增外部接口、数据库字段或迁移，也未发布到 test/SIT。

### 12.7 2026-07-30 简历恢复与批量导入第二批

- AI 解析失败的文件继续保存在本地，并以“待确认/待补全”候选人进入简历库；详情提供预览、下载、确认原件、更换简历、重新解析和手动补录，不再因模型 403 中断整个招聘操作。
- 重新解析再次失败时，接口返回“模型暂时不可用，可先手动补录；原始文件已保留”，不再只显示 HTTP 422；原件和当前失败状态继续保留。
- 手动编辑支持姓名、电话、邮箱、岗位、城市、工作年限、学历和技能。失败文件不再把 PDF 文件名预填成候选人姓名；只编辑其他字段时不覆盖完整学历明细，也不把未改动的技能评分统一重置。
- 同批文件逐项显示成功、重复、待补全和失败；普通失败项只重试该文件。重复文件显示匹配依据和已有候选人，用户可明确保留现有版本，或把新文件设为当前简历并自动归档旧版。
- 更换简历后若 AI 仍未识别，页面使用黄色提示说明“新原件已保留”，不再误报绿色成功；手动补录保存后继续返回历史简历版本，详情不会在保存后暂时丢失版本记录。
- 本批沿用本地 `:5190`、`:5010`、`:5100` 和 SQLite，不新增外部业务接口；历史版本 schema 使用既有 `20260729_12`，没有新增迁移。

### 12.8 2026-07-30 详情、数据提示与视觉规范第三批

- 候选人和面试详情统一为“简历 / 招聘流程”两页签；底部任务状态和“修改评价”等当前动作始终保留，切换页签不触发写操作。招聘专员与面试官复用同一展示结构，但面试官招聘流程继续由后端角色过滤并由前端 `interviewOnly` 限制为面试过程。常用查询条件直接展示，状态颜色和主次操作按钮保持一致；Offer 页面明确 OA 自动发起/同步尚未接入。
- 候选人列表继续只显示“相同文件/同名”提示，不自动合并、删除或覆盖；“隐藏本地演示数据”只过滤当前页显示，并明确提示不会删除数据。
- 所有当前真实路由主页面统一复用 `PageHeader`。本次补齐本月已入职、平均招聘周期、人力资源分析，以及总监驾驶舱、招聘进展、人才储备、审批与风险七个漏网页面；标题统一为同一字号、字重、行高和说明间距。
- 主页面的状态、范围和分类页签继续复用 `WorkspaceTabs` 的绿色实心选中态；除 Offer、招聘需求、候选人、面试和面试官页面外，总监“审批与风险”分类也已收口，不再使用灰底白色选中样式。
- 登录品牌继续统一为“智聘”，常见登录错误使用中文；安排面试只说明本地站内日程和待办，不展示当前未接入的外部能力承诺。
- 本批只有前端组件复用和文档更新，不新增后端接口、数据库字段、迁移或外部依赖，也没有执行评价、合并、删除和审批写操作。
