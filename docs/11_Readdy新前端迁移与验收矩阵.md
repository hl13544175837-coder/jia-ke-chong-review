...
| `/candidates` 简历库 | recruiter/manager/admin | `/candidates`、`/resume/*`、收藏、查重合并、匹配预览和批量入流程 | 支持先入公司人才库再匹配 Demand；全部、活动流程、公司人才库、个人收藏四个范围和多维条件均在分页前真实筛选。顶部补充条件等宽紧凑排列，范围页签位于列表上方；流程状态、学历、意向城市和当前阶段保留表头筛选，日期范围、来源、解析状态和排序置顶且不重复。当前阶段与兼容简历学历真实返回；单人/批量收藏、加入流程、淘汰后带原因重新启用已接通；manager/admin 可按完整联系方式安全合并无业务历史的重复档案 |
