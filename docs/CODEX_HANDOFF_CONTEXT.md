...
更新时间：`2026-07-24 12:16 +0800`

这份文档是新 Codex 窗口的第一入口。它记录本次任务的用户决定、代码来源、当前运行状态、验收结果、安全边界和后续接手顺序。若旧文档与这里冲突，本次独立整合版以本文和当前代码为准。

> **重要鉴权修正**：`5190` 已删除 Readdy 定时器假登录，接回公司 MD5 密码、网关 OAuth、Bearer Token、profile 工号、`X-Emp-Code`、`clientId=zhipin` 权限和路由守卫。五套产品角色界面已恢复，本地开发模式可在保持真实本地账号和 Token 不变的前提下预览切换；正式构建和公司模式不显示该入口。旧 `test-pgsgw` 当前不可用，默认地址已切到可访问的公司网关。公司技术角色按 GitLab `test` 基线由 profile 或 `VITE_DEFAULT_ROLE=admin` 提供；人力资源总监的正式身份映射和生产 RBAC 尚未补齐。公司原登录、Token、Apollo 和三方 Token 文件已做哈希冻结。完整证据见 `docs/evidence/2026-07-24-company-auth/README.md`。
...
- 登录鉴权是例外：必须沿用公司现有真实协议，不能使用假 Token；角色预览只允许由隔离启动脚本在本地开发模式开启。
...
| 服务 | 端口 | 定位 | 数据状态 |
| --- | ---: | --- | --- |
| `readdy-frontend` | `5190` | 最终验收主产品，页面和按钮以 ZIP 为准 | 公司鉴权；招聘需求接独立后端，其余业务数据仍以浏览器演示状态为主 |
...
公司 OAuth 协议已接入 `5190`；招聘需求列表、新增、编辑、关闭和恢复已通过公共 API 层接入本项目独立后端，生产环境仍未接入。其他正式业务 API、公司数据库、LLM 和视觉模型仍未接入。Apollo Secret、MCP SSO Token 和真实密钥只允许由环境、Apollo 或 CI 注入，禁止写进仓库。
...
- 接回公司登录鉴权并移除 Readdy 假登录；支持 Token、工号、权限菜单、路由守卫、会话失效和退出清理。
- 恢复招聘主管、招聘专员、系统管理员、人力资源总监和业务负责人 / 面试官五套产品界面及原始菜单边界。本地预览受 `DEV + VITE_ENABLE_ROLE_PREVIEW=true` 双重限制，正式环境仍按公司 profile 和菜单权限执行。
- 用户已确认试点流程为“业务填写需求 → HR 筛选 → 业务筛选 → 一面 → 二面 → Offer → 入职”；`interviewer` 产品文案调整为“业务负责人 / 面试官”，试点隐藏 AI 助手、BOSS 自动化和 KPI 标准。研发接线字段、动作和验收场景见 `docs/13_试点业务流程与研发接口交接.md`。
...
- 公司鉴权本地全链路通过，四角色守卫失败 0，Token 不进入 URL，退出后会话字段全部清除。
- 真实公司网关地址和浏览器 CORS 已验证；伪造账号探针收到公司返回的“用户名错误，请确认”。用户真实账号 `100001` 未由 Codex 收集密码或代为验证，仍需用户本人刷新后重试。
...
- `5190` 招聘需求模块已完成独立后端接线；简历、候选人、流程、面试、Offer 等模块尚未逐模块连接正式业务 API。
- 公司 OAuth 协议已接回，可访问的公司网关和跨域链路已验证，但用户真实账号尚未完成本人现场登录。
- 生产网关当前没有 `/zhipin-server/api/auth/me`，角色暂按公司 `test` 基线由 profile 或 `VITE_DEFAULT_ROLE=admin` 提供；这不是最终生产 RBAC。
...
2. 由用户本人用真实公司账号完成 `5190` 登录、权限菜单和退出现场验收，不记录密码和 Token。
