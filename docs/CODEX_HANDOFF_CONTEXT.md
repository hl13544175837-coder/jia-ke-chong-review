...
> **重要鉴权修正**：`5190` 已删除 Readdy 定时器假登录和假角色切换，接回公司 MD5 密码、网关 OAuth、Bearer Token、profile 工号、`X-Emp-Code`、`clientId=zhipin` 权限和路由守卫。旧 `test-pgsgw` 当前不可用，默认地址已切到可访问的公司网关。角色按 GitLab `test` 基线由 profile 或 `VITE_DEFAULT_ROLE=admin` 提供；生产网关尚未部署 `/zhipin-server/api/auth/me`，不能把兜底角色描述成正式 RBAC。公司原登录、Token、Apollo 和三方 Token 文件已做哈希冻结。完整证据见 `docs/evidence/2026-07-24-company-auth/README.md`。
...
- 接回公司登录鉴权，移除 Readdy 假登录和假角色切换；支持 Token、工号、权限菜单、路由守卫、会话失效和退出清理。本地验收桥可返回四角色；真实网关按 `test` 基线使用 profile 角色或默认 `admin`。
