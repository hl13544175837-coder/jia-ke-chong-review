...
“在线简历”与现有“简历库”是两个独立入口。外部 Agent 可选择性导入在线简历及完整聊天，也可把拿到的完整简历直接导入现有简历库；智聘只接收导入结果，不启动或控制外部 Agent。每份在线简历必须携带招聘专员明确选择且有权管理的 active `demand_id`；缺失、填错或越权时该条导入失败，系统不得自动归入其他需求。同一组织内拥有 `candidates` 菜单权限的招聘专员、招聘主管和管理员都可查看；仅导入人可编辑，其他可见账号可删除。删除不扣减历史导入数，并保留删除人与原导入人审计信息。
...
招聘专员日常使用时进入“在线简历”，点击“连接 Agent”，选择正在招聘的具体需求并填写 BOSS 账号，再点击“生成连接提示词”。系统会把接口地址、Demand ID、JD、BOSS 账号、两类导入规则和默认 30 天的限权凭证组装成一段完整提示词；点击“复制全部提示词”交给 Ego 即可。关闭弹窗后凭证明文不再保留。

本地 `127.0.0.1` 地址只适合同一台电脑上的 Ego；云端或其他电脑上的 Agent 必须使用已部署的 Test/SIT 接口地址。

外部 Agent 不使用招聘专员的完整登录凭证。技术调用仍可由招聘专员的正常登录 JWT 请求：
