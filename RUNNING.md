...
招聘专员侧边栏的“在线简历”与现有“简历库”是两个独立入口。外部 Agent 可选择性导入在线简历及完整聊天，也可把拿到的完整简历直接导入现有简历库；智聘只接收导入结果，不启动或控制外部 Agent。每份在线简历必须携带招聘专员明确选择且有权管理的 active `demand_id`；缺失、填错或越权时该条导入失败，系统不得自动归入其他需求。在线简历只对所属招聘专员本人展示，可编辑基础信息或手动删除；两个库不会自动关联、搬移、覆盖或删除。
