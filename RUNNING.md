...
简历库筛选区采用统一等宽、等高和紧凑间距：精确搜索、招聘需求、入库起止日期、来源、解析状态、排序在顶部，四个候选人范围位于列表上方；流程状态、学历、意向城市和当前阶段在表头筛选，重复入口不再保留。流程状态会区分招聘流程中、未进入流程、已淘汰、已入职和已转出，当前重新进入流程但曾被淘汰的候选人会额外标记“曾淘汰”。核心技能列和技能分筛选本期隐藏，岗位匹配与后端技能数据不受影响。
