...
招聘需求是主线，岗位/JD 是需求下的匹配画像。HR 可以在「招聘管理 → 用人需求」直接填写招聘岗位、用人部门、需求人数、优先级、期望完成时间和岗位职责/任职要求，系统会自动生成岗位画像用于匹配候选人；如果以前招过类似岗位，可以展开「复用已有岗位画像」选择已有画像。要做匹配，可以在需求卡片点击「匹配候选人」，也可以在「招聘岗位」列表进入对应画像。没有候选人时先上传简历；面试官为空时，管理员去用户管理创建或启用面试官账号，非管理员联系管理员处理。
...
如果候选人更适合另一个招聘需求，在候选人流程右侧使用「转入其他招聘需求」：选择目标招聘需求并填写原因后，系统会把当前需求追加为“已淘汰/转出”历史记录，把目标需求追加为“待筛选”当前记录，并写入审计事件。这样旧需求复盘能看到为什么转出，新需求看板能继续推进。
