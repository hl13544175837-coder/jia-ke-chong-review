...
候选人详情页可以导出当前候选人的 CSV，导出会记录操作者、候选人 ID、request_id、IP 和来源。轻量试点不做导出审批、导出原因和水印；如果同一账号 10 分钟内导出次数偏多，管理员审计页会把导出事件标红，方便事后检查。候选人删除是合规软删除：需要填写原因，系统会匿名化姓名/邮箱/电话/简历 JSON，删除原简历文件，并把候选人从列表和匹配中排除；历史流程、面试和审计事件保留，便于后续证明“谁在什么时候处理过删除请求”。
