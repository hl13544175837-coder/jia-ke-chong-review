...
简历上传页只负责把简历保存到简历库。支持 PDF、DOCX、JPG、PNG、WebP、GIF 和 ZIP；旧版 `.doc` 存在宏风险，系统会跳过并提示先转换。图片简历单张不超过 10 MB，服务端校验真实格式后通过百炼视觉模型识别；GIF 取首帧，结构化结果需要 HR 人工复核。上传成功后，如果要推进某个需求，去「简历库」选择具体「目标招聘需求」查看该需求对应岗位的适配候选人，再点击「加入所选需求」。筛选区里的「入需求流程状态」只用来区分候选人是否已进需求流程。候选人来源、内推人/猎头联系人和本次上传备注都是选填信息，默认收起，不影响上传。
...
`DASHSCOPE_API_KEY` 与 `DASHSCOPE_BASE_URL` 只在上传或重新解析图片简历时需要。未配置时，PDF/DOCX 仍按原 LLM 配置解析；图片候选人会保留为解析失败状态，配置补齐后可在候选人详情重新解析。不要把 Key、图片 Base64 或完整模型响应写入代码、日志或版本库。
