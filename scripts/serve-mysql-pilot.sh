...
load_mysql_pilot_env() {
  ...
  if mode="$(stat -f '%Lp' "$ENV_FILE" 2>/dev/null)"; then
    ...
    echo "Cannot verify mysql-pilot.env permissions; startup refused." >&2
    exit 1
  fi
  if [[ "$mode" != "600" ]]; then
    echo "mysql-pilot.env must have mode 600; startup refused." >&2
    exit 1
  fi
  ...
  if [[ "$PILOT_DATABASE_URL" != mysql://* && "$PILOT_DATABASE_URL" != mysql+pymysql://* ]]; then
    echo "PILOT_DATABASE_URL must select MySQL; startup refused." >&2
    exit 1
  fi
...
if [[ ! -x "$PYTHON_BIN" ]]; then
  echo "Missing project Python runtime; startup refused." >&2
  exit 1
fi

load_mysql_pilot_env
unset PILOT_SCHEMA_VERIFIED
"$PYTHON_BIN" "$APP_ROOT/backend/scripts/audit_mysql_pilot_schema.py" --require-compatible
export PILOT_SCHEMA_VERIFIED=true
export READDY_AUTH_MODE=local

trap cleanup EXIT INT TERM
"$SCRIPT_DIR/start-isolated-demo.sh"
echo "MySQL pilot services are running; press Ctrl+C to stop this isolated project."
while true; do
  sleep 3600
done
