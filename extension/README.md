...
Chromium 浏览器扩展，支持 Chrome / Tabbit 等浏览器一键采集 BOSS 直聘 Cookie，直接发送到智聘系统，解锁收件箱、岗位和简历导入能力。
...
1. 打开 Chrome 或 Tabbit，访问 `chrome://extensions/`
2. 右上角开启「开发者模式」
3. 点击「加载已解压的扩展程序」
...
1. 在安装扩展的同一个浏览器中打开 BOSS 直聘（`zhipin.com`），确保已登录
...
- **content.js** — 注入 BOSS 直聘页面，兜底提取可选的 `__zp_stoken__`
- **HTTP 直发** — 使用 fetch API 直接发送 Cookie 到智聘后端
- **后端校验** — 验证 Cookie 有效性，保存为激活账号

## `__zp_stoken__` 采集策略

`__zp_stoken__` 是 BOSS 直聘的客户端反爬 token，由页面 JS 运行时生成。当前产品保留的收件箱、岗位、简历下载和批量导入能力只强制要求 `wt2`、`wbg`、`zp_at`；扩展会尽量采集 `__zp_stoken__`，但不再把它作为导入账号的硬性条件。
...
| `activeTab` | 获取当前标签页信息 |
| `scripting` | 注入 Content Script 到 BOSS 页面，兜底提取可选的 `__zp_stoken__` |
| `storage` | 缓存服务器配置和登录 Token |
| `host_permissions: zhipin.com` | 监听 BOSS 直聘请求 |
| `host_permissions: localhost / 127.0.0.1` | 将 Cookie 发送到本地智聘后端 |
...
### Q: 缺少 `__zp_stoken__` 还能用吗？

A: 可以。当前必需 Cookie 是 `wt2`、`wbg`、`zp_at`，用于收件箱、岗位、简历下载和批量导入候选人。`__zp_stoken__` 是短效客户端 token，扩展会尽量采集，但不是当前导入账号的硬性条件。
...
- 扩展**仅**访问 `*.zhipin.com` 以及配置的智聘平台地址；本地开发默认发送到 `http://127.0.0.1:5001`
...
### v4.1.1
- 修复扩展未注入 `content.js` 导致复制 Cookie 缺少 `__zp_stoken__` 的问题。
- 默认智聘地址改为本地开发后端 `http://127.0.0.1:5001`，并补充 Chrome / Tabbit 访问本地 API 所需权限。
- 将 `__zp_stoken__` 从必需项调整为可选项，和后端收件箱/简历导入能力保持一致。
