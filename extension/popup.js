...
// 收件箱、岗位、简历等 Tier-1 功能只需要服务端会话 Cookie。
// __zp_stoken__ 是短效客户端 token，仅作为可选增强项采集。
const REQUIRED_COOKIES = ['wt2', 'wbg', 'zp_at'];
const OPTIONAL_COOKIES = ['__zp_stoken__'];
...
async function readCookies(tab) {
  ...
  // 从 BOSS 页面上下文兜底提取 __zp_stoken__。该 token 通常由页面 JS 生成，
  // 未必会出现在最近一次请求头里。
  try {
    let stokenResp = await chrome.tabs.sendMessage(tab.id, { action: 'getStoken' });
    if (!stokenResp || !stokenResp.stoken) {
      await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ['content.js'] });
      stokenResp = await chrome.tabs.sendMessage(tab.id, { action: 'getStoken' });
    }
    if (stokenResp && stokenResp.stoken) {
      cookieMap.__zp_stoken__ = stokenResp.stoken;
    }
  } catch (e) {
    console.warn('提取 __zp_stoken__ 失败:', e);
  }
  ...
  if (total === 0) {
    ...
    const hint = missing.includes('__zp_stoken__')
      ? '请在 BOSS 招聘端页面点开「推荐/沟通」并等待几秒，再点「刷新状态」。'
      : missing.includes('zp_at') || missing.includes('wt2')
...
function formatCookies(cookies) {
  return REQUIRED_COOKIES
    .concat(OPTIONAL_COOKIES)
    .concat(Object.keys(cookies).filter(k => !REQUIRED_COOKIES.includes(k) && !OPTIONAL_COOKIES.includes(k)))
    .filter(k => cookies[k])
    .map(k => k + '=' + cookies[k])
    .join('; ');
}
