...
- 旧版 `.doc` 因宏风险会被跳过；后端提供受控的批次撤回接口，但当前上传页尚无整批撤回入口
- 候选人来源、内推人/猎头联系人和批次备注默认作为选填信息收起，减少上传主流程干扰
- 候选人详情支持受控导出 CSV；打开详情、导出和删除都会进入审计日志，高频导出会在审计 API 中标记为 `warning`，当前管理员页尚未展示完整告警上下文
- 候选人删除采用软删除 + 匿名化：清空姓名/邮箱/电话/简历 JSON，移除原简历文件，历史流水保留用于审计

### 💼 招聘需求与岗位画像
- 招聘需求是业务主线：OA 或 HR 手工创建的是一次真实招人任务，包含用人部门、招聘岗位、人数、优先级和期望完成时间
- 岗位/JD 是需求下的匹配画像：当前页面先创建岗位画像，再在用人需求中选择没有未结束需求的画像；后端创建接口仍支持 `job_title + jd_text` 自动建画像，同一画像不能同时承接两条待确认/进行中/暂停需求
...
- 候选人流程支持按待办直达阶段，默认避开空阶段；重复推进、重复安排面试和重复提交反馈会被后端复用已有记录；同一面试官同一时间不能被排两场不同面试；创建面试安排后会先给面试官生成站内通知；“修正阶段”用于误推进或误淘汰补救，必须填写修正原因，历史流水保留，当前阶段和 BI 当前存量按最新阶段计算；后端支持受控的跨需求转入，但当前候选人页面尚无入口
- 候选人负责人转派改为选择招聘专员姓名并填写转派原因，不再要求输入用户 ID
- 招聘经理/管理员可转派需求与岗位画像负责人；必须选择同组织启用中的招聘专员并填写原因，需求和画像同步交接，但已有候选人负责人不会自动变化，需要时另行转派候选人

### 📊 数据看板
- 团队当前阶段分布（当前流程人数不含已入职/已淘汰，全流程入职占比按活跃流程 + 归档结果去重计算）
- 专员当前负责盘子、渠道质量、简历消化和面试责任归因，周期指标按本期入库简历 cohort 统计；候选人负责人转派会把候选人本期统计整体归入新负责人，单独转派需求/岗位不会改候选人归属；这些指标不作为事件时点历史绩效
...
版本下限以当前锁定依赖为准：`pandas 3.x` 需要 Python 3.11+，Vite 8 需要 Node.js 20.19+ 或 22.12+。旧版运行时即使能安装部分依赖，也不作为受支持环境。
...
│   ├── app/api/      API 蓝图（auth/resume/jobs/demands/match/pipeline/interview/bi/agent/boss/oa/talent_maps/notifications/admin/candidates）
│   ├── app/services/ 业务服务（agent / match / resume / interview / boss / oa_merak）
│   └── run.py        启动入口
