...
def test_interview_uniqueness_revision_adds_primary_slot_and_unique_indexes(tmp_path):
    ...
    with engine.connect() as connection:
        ...
        ).scalar_one() == "20260722_06"
    engine.dispose()


def test_event_source_revision_accepts_controlled_interview_entry_name(tmp_path):
    path = tmp_path / "event-source.db"
    connection = sqlite3.connect(path)
    connection.executescript(
        """
        CREATE TABLE alembic_version (version_num VARCHAR(32) NOT NULL);
        INSERT INTO alembic_version (version_num) VALUES ('20260722_05');
        CREATE TABLE events (
            id INTEGER PRIMARY KEY,
            source VARCHAR(20) NOT NULL
        );
        INSERT INTO events (id, source) VALUES (1, 'interview_access_link');
        """
    )
    connection.commit()
    connection.close()
    ...
    assert source_column["type"].length == 64
    with engine.connect() as upgraded:
        assert upgraded.execute(text("SELECT source FROM events WHERE id = 1")).scalar_one() == (
            "interview_access_link"
        )
        assert upgraded.execute(text("SELECT version_num FROM alembic_version")).scalar_one() == (
            "20260722_06"
        )
    engine.dispose()
