...
export default function JobDetailPanel({ req, onClose, highlightCandidateName }: JobDetailPanelProps) {
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activeTab === 'info' && (
            <>
              ...
              {/* Basic Info */}
              <div>
                <h3 className="text-sm font-semibold text-foreground-900 mb-3">基本信息</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">所属部门</p>
                    <p className="text-sm font-medium text-foreground-900">{req.department}</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">工作城市</p>
                    <p className="text-sm font-medium text-foreground-900">{req.city}</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">招聘负责人</p>
                    <p className="text-sm font-medium text-foreground-900">{req.owner}</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">薪资范围</p>
                    <p className="text-sm font-medium text-foreground-900">{req.salaryRange || '未设置'}</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">计划招聘</p>
                    <p className="text-sm font-medium text-foreground-900">{req.headcount}人</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">已入职</p>
                    <p className="text-sm font-medium text-foreground-900">{req.filled}人</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">剩余HC</p>
                    <p className="text-sm font-medium text-foreground-900">{req.headcount - req.filled}人</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">招聘开始日期</p>
                    <p className="text-sm font-medium text-foreground-900">{req.startDate || req.createdAt}</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3">
                    <p className="text-xs text-foreground-400 mb-1">计划关闭日期</p>
                    <p className="text-sm font-medium text-foreground-900">{req.deadline || '未设置'}</p>
                  </div>
                  <div className="bg-background-50 rounded-lg p-3 col-span-2">
                    <p className="text-xs text-foreground-400 mb-1">关联招聘需求</p>
                    <p className="text-sm font-medium text-foreground-900">{displayTitle}招聘需求</p>
                  </div>
                </div>
              </div>

              {/* JD */}
              {(req.responsibilities || req.requirements) && (
                <div>
                  {req.responsibilities && (
                    <div className="mb-4">
                      <h3 className="text-sm font-semibold text-foreground-900 mb-2">岗位职责</h3>
                      <div className="bg-background-50 rounded-lg p-4 text-sm text-foreground-700 leading-relaxed whitespace-pre-line">
                        {req.responsibilities}
                      </div>
                    </div>
                  )}
                  {req.requirements && (
                    <div>
                      <h3 className="text-sm font-semibold text-foreground-900 mb-2">任职要求</h3>
                      <div className="bg-background-50 rounded-lg p-4 text-sm text-foreground-700 leading-relaxed whitespace-pre-line">
                        {req.requirements}
                      </div>
                    </div>
              ...
              {/* Interview Process */}
              {req.interviewProcess && (
                <div>
                  <h3 className="text-sm font-semibold text-foreground-900 mb-2">面试流程</h3>
                  <div className="bg-background-50 rounded-lg p-4">
                    <div className="flex items-center flex-wrap gap-2">
                      {req.interviewProcess.split(' → ').map((step, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <span className="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-full bg-primary-100 text-primary-700 whitespace-nowrap">
                            {step.trim()}
                          </span>
                          {i < req.interviewProcess!.split(' → ').length - 1 && (
              ...
              {/* Stats */}
              <div>
                <h3 className="text-sm font-semibold text-foreground-900 mb-3">招聘进展</h3>
                <div className="grid grid-cols-4 gap-3">
                  <div className="bg-secondary-50 rounded-lg p-3 text-center border border-secondary-200">
                    <p className="text-xl font-bold text-secondary-700">{pendingCount}</p>
                    <p className="text-xs text-foreground-500 mt-0.5">待筛选</p>
                  </div>
                  <div className="bg-accent-50 rounded-lg p-3 text-center border border-accent-200">
                    <p className="text-xl font-bold text-accent-700">{interviewCount}</p>
                    <p className="text-xs text-foreground-500 mt-0.5">面试中</p>
                  </div>
                  <div className="bg-primary-50 rounded-lg p-3 text-center border border-primary-200">
                    <p className="text-xl font-bold text-primary-600">{offerCount}</p>
                    <p className="text-xs text-foreground-500 mt-0.5">Offer中</p>
                  </div>
                  <div className="bg-primary-50 rounded-lg p-3 text-center border border-primary-200">
                    <p className="text-xl font-bold text-primary-700">{hiredCount}</p>
                    <p className="text-xs text-foreground-500 mt-0.5">已入职</p>
                  </div>
                </div>
              </div>
