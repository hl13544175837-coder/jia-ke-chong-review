...
assert.match(companyAuth, /userId\?:\s*number/, 'profile 必须支持后端真实数字用户 ID');
assert.match(companyAuth, /user_id:\s*userId/, '会话必须使用 profile userId');
assert.doesNotMatch(companyAuth, /Number\(empCode\)/, '公司工号不得强制转成业务用户 ID');
...
assert.doesNotMatch(layout, /previewEnabled|setPreviewRole|预览角色/, '主产品不得保留本地假角色切换入口');
...
assert.doesNotMatch(productRole, /setPreviewRole|VITE_ENABLE_ROLE_PREVIEW|zhipin-current-role/, '角色上下文只能读取真实登录身份');
assert.match(productRole, /useCompanyAuth/, '产品角色必须来自公司会话');
