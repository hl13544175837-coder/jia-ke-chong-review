...
assert.match(api, /authHeaders\(\)/, 'Original file requests should reuse bearer and employee-code authentication');
assert.match(api, /\$\{API_BASE\}\/resume\/\$\{candidateId\}\/original\/\$\{mode\}/, 'Original files should call the protected route family');
